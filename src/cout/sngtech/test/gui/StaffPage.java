...
public class StaffPage extends JPanel {
	...
	private static final long serialVersionUID = 3179859906602087091L;
	...
	public StaffPage(MainFrame main) {
		...
		JLabel lblSubmittedEventDetails = new JLabel("Created Event Details");
		...
		JLabel lblEventList = new JLabel("Available Events For Students");
		lblEventList.setFont(new Font("Tahoma", Font.BOLD, 16));
		lblEventList.setBounds(49, 80, 331, 16);
		add(lblEventList);
		
		JLabel lblSubmitCcaEvent = new JLabel("Create CCA Event");
		...
		btnConfirm.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for(Event event : main.getEventController().getCatalogue().getEvents()) {
		...
		JButton btnStudentView = new JButton("Switch to Student View");
		btnStudentView.setBounds(359, 44, 169, 25);
		btnStudentView.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				main.getPageManager().showPage(PagesList.LOGIN_PAGE);
			}
		});
		add(btnStudentView);
		
		JLabel lblNewLabel_1 = new JLabel("Staff Home Page");
		lblNewLabel_1.setFont(new Font("Tahoma", Font.BOLD, 18));
		lblNewLabel_1.setBounds(367, 13, 153, 16);
		add(lblNewLabel_1);
	}
}
